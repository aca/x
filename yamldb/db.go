...
// Package yamldb provides a trivial "database": a Go object saved to
// disk as yaml.
package yamldb
...
// DB is a database backed by a yaml file.
...
func Open[T any](path string) (*DB[T], error) {
	...
	var val T
	if err := yaml.Unmarshal(bs, &val); err != nil {
		return nil, err
	}
